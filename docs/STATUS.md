...
|---|---|---|---|
| 1 | Clean checkout builds and installs | Partial | Local normal release APK builds with Cast (69.9 MB, debug-key signed); clean-checkout normal-install workflow added, native install proof pending |
...
- `flutter test --no-pub --reporter expanded --dart-define=RENDER_UI=true`: **551 passed** with an actual local private adapter selected for TLS client tests; no LAN skips. Full `flutter analyze --no-pub` reports no issues. Tests include five rendered layouts, 200% text, long player times, landscape keyboards, screen selection and paywall accessibility.
