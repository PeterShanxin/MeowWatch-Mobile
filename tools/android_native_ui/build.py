#!/usr/bin/env python3
"""Build the independent observer APK using only the installed JDK/Android SDK."""
...
DEFAULT_OUTPUT = Path("build/android-native-ui")


def build(sdk: Path, java_home: Path, output: Path, *, platform: int = 35,
          build_tools: str = "36.0.0") -> Path:
    if output.exists() and any(output.iterdir()):
        raise ValueError("observer build directory must start empty")
    ...
    def run(label: str, arguments: list[str | Path]) -> None:
        ...
        if completed.returncode:
            raise RuntimeError(f"observer build failed at {label}; inspect its local build log")
    ...
    apk = output / "native-ui-observer.apk"
    ...
    targets = re.findall(r':targetPackage\([^\n=]+\)="([^"]+)"', manifest)
    if targets != ["com.meowwatch.native_ui_observer"]:
        raise RuntimeError("packaged observer manifest does not target its own package")
    (output / "build.json").write_text(json.dumps({
        ...
        "instrumentationTarget": "com.meowwatch.native_ui_observer",
        "commands": commands,
