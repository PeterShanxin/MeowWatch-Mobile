...
- Both portrait cases require the exact source title immediately before the unique **Playing on this phone** caption in the same player-body accessibility container, plus the 52-second native duration, one player timeline and one enabled playback control. The header may repeat the filename; a header-only match, wrong or hidden body title, duplicate body title, or different target cannot pass. Fresh native observer snapshots must show playing time advancing by at least two displayed seconds in the same production process. Screenshots and XML capture both observations; a review label or Play icon alone cannot pass.
...
When a complete native tree fails a playback check, `*-last-rejected.xml` retains the most recent rejected tree and its matching `rejectedXmlSha256` appears in that attempt's wait record. A later capture failure records no XML hash and never treats the retained rejected tree as a fresh sample.
...
The gate does not prove network-request absence before confirmation, activity recreation, early warm-intent ordering while the engine is unavailable, landscape playback, or a completed invitation join. Those boundaries remain separate.

Run [35230867474](https://github.com/PeterShanxin/MeowWatch-Mobile/actions/runs/35230867474) fails all three normal-APK variants after HTTPS confirmation: the player shows `trailer.mp4` in both the header and body, but the former assertion required a globally unique filename. These are failed gates, not playback passes. The body-scoped assertion is covered by `fixtures/native-api35-paused-player.xml`, an unchanged API 35 release native capture from run [35223362735](https://github.com/PeterShanxin/MeowWatch-Mobile/actions/runs/35223362735), SHA-256 `a20d0080c7dd2ad963cefd9344b908b2743e86993eb0e6bb583a3fcd7844176b`. The regression explicitly changes only that fixture's old header text to reproduce the current duplicate label and checks negative mutations. This modified test input is not a new runtime capture; fresh native execution is still required. The failed run retained capture hashes and screenshots, but not the rejected complete XML, motivating the explicit rejected-tree record above.
